...
class AboutService
{
    ...
    public function createAbout($request)
    {
        ...
        return $About ? $this->success($About, 201, 'About created successfully') : $this->error(null, 404, 'can not create About');
    }

    public function updateAbout($id, $request)
    {
        $About = $this->update($this->model, $request, $id);
        return $About ? $this->success($About, 200, 'About updated successfully') : $this->error(null, 404, 'can not update About');
    }

    public function deleteAbout($id)
    {
        $About = $this->destroy($this->model, $id);
        return $About ? $this->success($About, 200, 'About deleted successfully') : $this->error(null, 404, 'can not delete About');
    }
}
